...
async function approve(tokenAddress, tokenAmount, chainId){
    try{
        const response = await axios.get(`https://api.1inch.exchange/v3.0/${chainId}/approve/calldata?tokenAddress=${tokenAddress}&amount=${tokenAmount}`)
        if(response.data){
            data = response.data
            data.gas = 1000000
